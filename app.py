...
# Only import after potential installation
if install_playwright():
    from crawl4ai import AsyncWebCrawler, BrowserConfig, CacheMode, CrawlerRunConfig
    from crawl4ai.extraction_strategy import LLMExtractionStrategy
    from pydantic import BaseModel
else:
    st.error("Could not proceed due to Playwright installation failure.")
    st.stop()
...
if st.button("Start Scraping"):
    if url_to_scrape and instruction_to_llm:
        with st.spinner("Scraping in progress..."):
            try:
                # Wrap asyncio.run in a try-except block for more robust error handling
                ...
                st.error("Please check your API keys, Playwright installation, and ensure all dependencies are correctly installed.")
    else:
